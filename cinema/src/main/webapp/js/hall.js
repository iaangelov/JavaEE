				...
				statusCode : {
					200 : function(tickets) {
						...
						var settings = {
							...
							rowCssPrefix : 'row-',
							colCssPrefix : 'col-',
							seatWidth : ($($('#place').parent()[0]).width() - $(
									$('#place').parent()[0]).width() / 20) / 20,
							seatHeight : ($($('#place').parent()[0]).height() - $(
									$('#place').parent()[0]).height() / 12) / 12,
							seatCss : 'seat',
							...
							for (i = 0; i < settings.rows; i++) {
								for (j = 0; j < settings.cols; j++) {
									...
									str.push('<li class="'
											...
											+ '"'
											+ 'style="top:'
											+ (i * settings.seatHeight)
													.toString()
											+ 'px;left:'
											+ (j * settings.seatWidth)
													.toString() + 'px">'
											+ '<a title="' + seatNo + '">'
											+ seatNo + '</a>' + '</li>');
								}
